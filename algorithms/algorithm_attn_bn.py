...
class ZhangNet(nn.Module):
    def __init__(self, bands, number_of_classes):
        ...
        self.weighter = nn.Sequential(
            nn.Linear(self.bands, self.bands),
            nn.ReLU(),
            nn.Linear(self.bands,self.bands),
...
class Algorithm_attn_bn(Algorithm):
    ...
    def report_stats(self, channel_weights, sparse_weights, epoch, mse_loss, l1_loss, lambda_value, l2_loss, alpha, loss):
        ...
        channel_weights_mean = torch.mean(channel_weights, dim=0)
        sparse_weights_mean = torch.mean(sparse_weights, dim=0)

        min_cw = torch.min(channel_weights_mean).item()
        min_s = torch.min(sparse_weights_mean).item()
        max_cw = torch.max(channel_weights_mean).item()
        max_s = torch.max(sparse_weights_mean).item()
        avg_cw = torch.mean(channel_weights_mean).item()
        avg_s = torch.mean(sparse_weights_mean).item()

        l0_cw = torch.norm(channel_weights_mean, p=0).item()
        l0_s = torch.norm(sparse_weights_mean, p=0).item()
        ...
        self.reporter.report_epoch(epoch, mse_loss, l1_loss, lambda_value, l2_loss, alpha, loss,
                                   ...
                                   min_s, max_s, avg_s,
                                   l0_cw, l0_s,
                                   selected_bands, sparse_weights_mean)

    def get_indices(self, sparse_weights):
        sparse_weights = torch.mean(sparse_weights, dim=0)
