...
class Algorithm_lw_relu_custom(Algorithm):
    ...
    def get_selected_indices(self):
        ...
        for epoch in range(self.total_epoch):
            ...
            for batch_idx, (X, y) in enumerate(dataloader):
                ...
                l1_loss = self.custom_regularization(channel_weights)
    ...
    def custom_regularization(self, weights):
        threshold = 0.8
        reg_loss = torch.where(weights < threshold, torch.abs(weights), torch.zeros_like(weights))
        return reg_loss.sum()
